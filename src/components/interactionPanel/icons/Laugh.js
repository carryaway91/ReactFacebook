import React from 'react';


const Laugh = props => {
    return (
        <svg  x="0px" y="0px" viewBox="0 0 512 512" width={props.w} height={props.w}>
        <circle fill='#FFCA28' cx="256" cy="256" r="256"/>
        <path fill="#FAFAFA" d="M438.016,294.336c-3.008-4-7.744-6.336-12.736-6.336H86.72c-5.024,0-9.728,2.336-12.736,6.336
            c-3.04,4-4.032,9.152-2.656,14.016C94.464,390.56,170.4,448,256,448s161.536-57.44,184.672-139.648
            C442.016,303.488,441.056,298.336,438.016,294.336z"/>
        <g>
        <path fill="#6D4C41" d="M368,256c-3.328,0-6.72-1.056-9.6-3.2l-64-48c-4.032-3.008-6.4-7.776-6.4-12.8s2.368-9.792,6.4-12.8
            l64-48c7.04-5.28,17.056-3.904,22.4,3.2c5.28,7.072,3.84,17.088-3.232,22.4L330.656,192l46.944,35.2
            c7.072,5.312,8.512,15.328,3.232,22.4C377.664,253.792,372.832,256,368,256z"/>
        <path fill="#6D4C41" d="M144,256c-4.864,0-9.664-2.208-12.8-6.4c-5.312-7.072-3.872-17.088,3.2-22.4l46.944-35.2
            L134.4,156.8c-7.072-5.312-8.512-15.328-3.2-22.4c5.312-7.072,15.328-8.512,22.4-3.2l64,48c4.032,3.008,6.4,7.776,6.4,12.8
            s-2.368,9.792-6.4,12.8l-64,48C150.72,254.944,147.36,256,144,256z"/>
    </g>
    </svg>
    );
};


export default Laugh;
